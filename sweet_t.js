...
const KINESCOPE_VIDEO_URL = 'https://kinescope.io/vgBf24eNiMP3WitUU6SrnP';
const KINESCOPE_EMBED_URL = 'https://kinescope.io/embed/vgBf24eNiMP3WitUU6SrnP';
...
function loadKinescopeFactory() {
  ...
  return new Promise((resolve, reject) => {
    const existing = document.querySelector('script[data-kinescope-api]');
    if (existing) {
      existing.addEventListener('load', () => resolve(window.Kinescope.IframePlayer));
    ...
    script.onload = () => resolve(window.Kinescope.IframePlayer);
...
async function startKinescopeFilm() {
  ...
  let wrap = document.getElementById('kinescope-wrap');
  if (wrap) {
    wrap.remove();
  }
  ...
  try {
    const factory = await loadKinescopeFactory();
...
async function stopKinescopeFilm() {
  ...
  kinescopePlayer = null;
  const wrap = document.getElementById('kinescope-wrap');
  if (wrap) {
    wrap.remove();
  }
}
...
async function experimentInit() {
  ...
  // Kinescope вместо локального MovieStim / mp4
  film = {
    name: 'film',
...
function assignmentRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of assignmentComponents)
...
function beforeRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of beforeComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function after_allRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of after_allComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function after_alllRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of after_alllComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function after_allllRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of after_allllComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function open_responseRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of open_responseComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function debrief_group_1RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of debrief_group_1Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function debrief_group_2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of debrief_group_2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function debrief_group_3RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of debrief_group_3Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function thanxRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of thanxComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
async function quitPsychoJS(message, isCompleted) {
  ...
  // Отправка всех полей в Google Sheets (GitHub Pages не умеет писать CSV на сервер)
  try {
...
async function sendDataToGoogleSheets(isCompleted) {
  ...
  // text/plain — надёжный способ POST в Apps Script из браузера
  await fetch(url, {
    method: 'POST',
    mode: 'no-cors',
